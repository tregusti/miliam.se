...
var MetaSerializer = require('./meta-serializer');
...
var Entry = function(path) {
  ...
  var entry = this;
  fs.readFile(Path.join(path, 'info.txt'), 'utf8', function(err, data) {
    if (err) return
    MetaSerializer.deserialize(entry, data)

    entry.fire('load')
  });
};
Entry.prototype = Object.create(Observable.prototype);
